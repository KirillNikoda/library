...
export class AuthService {
  ...
  async login(loginUserDto: LoginUserDto): Promise<UserEntity & {token: string}> {
    const isValid = await this.usersService.checkIfUserExists(
      loginUserDto.email ? loginUserDto.email : loginUserDto.name,
    );
    ...
    const isCorrectPassword = await bcrypt.compare(loginUserDto.password, isValid.password);
